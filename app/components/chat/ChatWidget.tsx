  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-40 rounded-full bg-brand-600 p-4 text-white shadow-lg hover:bg-brand-700 transition-transform hover:scale-105"
      >
      ...
      >
        <div className="flex h-16 items-center justify-between border-b bg-gray-50 px-4">
          ...
          <button aria-label="Cerrar mensajes" onClick={() => setIsOpen(false)} className="text-gray-500 hover:text-gray-700">
            <X className="h-6 w-6" />
          </button>
        </div>
          ...
          {!selectedRoomId ? (
            ...
            <div className="flex flex-col h-full">
              <div className="bg-gray-100 p-2 text-sm border-b">
                <button onClick={() => setSelectedRoomId(null)} className="text-brand-600 hover:text-brand-800 font-medium">
                  &larr; Volver a salas
                </button>
              </div>
              <ChatWindow
