...
export default function EvaLayout({
  ...
  return (
    <div className="portal-shell flex h-screen overflow-hidden bg-[#f4f6f8]">
      ...
      <header className="portal-topbar fixed top-0 inset-x-0 h-16 flex items-center justify-between bg-[#111827] text-white px-4 sm:px-6 z-50 shadow-md">
        <div className="flex items-center gap-4">
          <button aria-label="Abrir menú del portal" aria-expanded={isMobileMenuOpen} className="md:hidden text-white hover:bg-white/10 p-2 rounded-md transition-colors" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
            <Menu className="h-6 w-6" />
          </button>
          <Link href={`/${userRole}`} className="portal-brand hover:text-gray-200 transition-colors">
            Colegio Militar <span> / EVA</span>
          </Link>
        </div>
        <div className="flex items-center gap-4">
          <button
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white ring-2 ring-[#111827]">
                {unreadCount > 9 ? '9+' : unreadCount}
              </span>
            )}
          </button>
          <Link href={`/${userRole}/perfil`} className="h-8 w-8 rounded-full bg-white text-[#111827] flex items-center justify-center font-bold text-sm hover:ring-2 hover:ring-white transition-all cursor-pointer">
            {userName.charAt(0)}
          </Link>
        </div>
      </header>
      ...
      <aside className={`portal-sidebar fixed inset-y-0 top-16 left-0 z-40 w-64 transform bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out md:relative md:top-0 md:mt-16 md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <nav className="flex-1 space-y-1 px-4 py-6">
            <p className="portal-sidebar-label">Portal · {userRole}</p>
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex items-center rounded-md px-3 py-2.5 text-sm font-medium transition-colors ${
                    isActive ? 'bg-[#fef3c7] text-[#111827] font-bold border-l-4 border-[#111827]' : 'text-gray-700 hover:bg-gray-100'
                  }`}
                >
                  {link.name}
                </Link>
          ...
          <div className="p-4 border-t border-gray-700 bg-gray-900">
            ...
            <p className="text-xs text-gray-300 uppercase mt-1">{userRole}</p>
            <form action="/api/logout" method="POST" className="mt-4">
              <button type="submit" className="text-xs font-semibold text-brand-200 hover:text-white transition-colors">
                Cerrar Sesión
              </button>
            </form>
          </div>
      ...
      {/* Main Content Area */}
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden pt-16">
        {/* Page Content */}
        <main className="portal-content flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          {children}
        </main>
      </div>
      ...
      >
        <div className="flex h-16 items-center justify-between border-b bg-gray-50 px-4">
          ...
          <button aria-label="Cerrar mensajes" onClick={() => setIsChatOpen(false)} className="text-gray-500 hover:text-gray-700">
            <X className="h-6 w-6" />
          </button>
        </div>
          ...
          {!selectedRoomId ? (
            ...
            <div className="flex flex-col h-full">
              <div className="bg-gray-100 p-2 text-sm">
                <button onClick={() => setSelectedRoomId(null)} className="text-brand-600 hover:underline">
                  &larr; Volver a salas
                </button>
              </div>
              <ChatWindow
